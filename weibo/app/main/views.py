...
def index():
    ...
    if form.validate_on_submit() and current_user.can(Permission.WRITE_ARTICLES):
        ...
        db.session.commit()
        blockchain.blockChainPut("weibo.post."+str(_post.id),str(_post.to_blockchain()))
        blockchain.blockChainTransferToken("coinbase","weibo.coin."+ str(_post.author_id),str(random.randint(3, 7)))
...
def user(username):
    ...
    pagination = _user.posts.order_by(Post.timestamp.desc()).paginate(
        ...
        error_out=False)
    posts = pagination.items
    WBcoin=str(blockchain.blockChainbalanceToken("weibo.coin."+str(_user.id)),encoding='utf8')
...
def post(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        blockchain.blockChainPut("weibo.comment."+str(comment.id),str(comment.to_blockchain()))
        blockchain.blockChainTransferToken("coinbase","weibo.coin."+ str(comment.author_id),str(random.randint(0, 2)))
        flash(_l('您已经成功提交评论'))
...
def delete(id):
    ...
    db.session.commit()
    blockchain.blockChainDelete("weibo.post." + str(_post.id))
    flash(_l('您已删除该微博'))
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        blockchain.blockChainPut("weibo.post."+str(current_user.id),str(current_user.to_blockchain()))
        flash(_l('您已经更新了个人资料'))
