...
from . import api
from .. import blockchain,onBlockchain
import random
restful_api = Api(api)
...
class PostApi(Resource):
    ...
    def post(self):
        ...
        db.session.add(post)
        db.session.commit()
        blockchain.blockChainPut("weibo.post."+str(post.id),str(post.to_blockchain()))
        blockchain.blockChainTransferToken("coinbase","weibo.coin."+ str(post.author_id),str(random.randint(3,7)))
        return {'status': 200}
...
class SetMode(Resource):
    def post(self, mode):
        return onBlockchain.setMode(mode)

class GetMode(Resource):
    def get(self):
        return onBlockchain.getMode()
...
restful_api.add_resource(SetMode, '/mode/<string:mode>')
restful_api.add_resource(GetMode, '/mode')
